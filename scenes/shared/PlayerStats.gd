...
func take_damage(amount: float) -> void:
	...
	if health <= 0.0:
		print("PlayerStats: Health depleted; trigger death handling.")
...
func spend_stamina(amount: float) -> bool:
	if amount <= 0.0:
		return false
	if stamina < amount:
		print("PlayerStats: Not enough stamina (%f/%f)" % [stamina, amount])
		return false
...
func spend_magicka(amount: float) -> bool:
	if amount <= 0.0:
		return false
	if magicka < amount:
		print("PlayerStats: Not enough magicka (%f/%f)" % [magicka, amount])
		return false
...
func gain_xp(amount: float) -> void:
	...
	while xp >= xp_needed:
		...
		level += 1
		print("PlayerStats: Level up! Level %d" % level)
...
func _modify_core_stat(stat_name: String, delta: float, clamp_min: float, clamp_max: float) -> void:
	if not _has_core_stat(stat_name):
		push_error("PlayerStats: Unknown stat '%s'" % stat_name)
		return
...
func _log_stat_change(stat_name: String, old_value: float, new_value: float) -> void:
	print("PlayerStats: %s %.2f -> %.2f" % [stat_name.capitalize(), old_value, new_value])

func _log_all_stats(context: String) -> void:
	print("PlayerStats (%s): Health %.0f / Stamina %.0f / Magicka %.0f / Level %d / XP %.0f" % [
